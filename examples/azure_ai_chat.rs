...
use litellm_rs::core::providers::azure_ai::{AzureAIProvider, AzureAIConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if config.base.api_base.is_none() {
        ...
        eprintln!("Please set it with: export AZURE_AI_API_BASE=https://your-resource.cognitiveservices.azure.com");
        return Ok(());
    }
    ...
    streaming_request.messages = vec![
        ChatMessage {
            role: MessageRole::User,
            content: Some(MessageContent::Text(
    ...
    match provider.chat_completion_stream(streaming_request, context).await {
        Ok(mut stream) => {
    ...
    match provider.chat_completion(conversation_request, context).await {
        Ok(response) => {
            if let Some(content) = response.first_content() {
