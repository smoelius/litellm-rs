...
use litellm_rs::core::providers::xai::{XAIProvider, XAIConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        messages: vec![
            ChatMessage {
                ...
                content: Some(MessageContent::Text(
                    "You are a helpful assistant that can access web information when needed.".to_string()
                )),
                name: None,
                function_call: None,
                tool_calls: None,
    ...
    match provider.chat_completion(reasoning_request, RequestContext::default()).await {
        Ok(response) => {
            println!("\nReasoning Response:");
            if let Some(choice) = response.choices.first() {
                if let Some(ref content) = choice.message.content {
    ...
    match provider.calculate_cost("grok-2", 1000, 500).await {
        Ok(cost) => println!("Cost for 1000 input + 500 output tokens on grok-2: ${:.4}", cost),
        Err(e) => println!("Cost calculation failed: {}", e),
    }

    match provider.calculate_cost("grok-2-mini", 1000, 500).await {
        Ok(cost) => println!("Cost for 1000 input + 500 output tokens on grok-2-mini: ${:.4}", cost),
        Err(e) => println!("Cost calculation failed: {}", e),
    }
