...
impl AzureChatHandler {
    ...
    async fn build_headers(&self) -> Result<HeaderMap, AzureError> {
        ...
        if let Some(api_key) = self.config.get_effective_api_key().await {
            ...
            return Err(ProviderError::authentication("azure", "No API key available".to_string()));
        }
    ...
    pub fn transform_response(&self, response: Value, model: &str) -> Result<ChatResponse, AzureError> {
        let choices = response["choices"]
            .as_array()
            .ok_or_else(|| ProviderError::serialization("azure", "Missing choices array".to_string()))?
            .iter()
            .map(|choice| {
                let message = &choice["message"];
                let content = message["content"].as_str().map(|s| MessageContent::Text(s.to_string()));
                ...
                ChatChoice {
                    ...
                    finish_reason: choice["finish_reason"].as_str().map(|reason| {
                        match reason {
        ...
        Ok(ChatResponse {
            ...
            choices,
            usage,
            system_fingerprint: response["system_fingerprint"].as_str().map(|s| s.to_string()),
        })
    }
                ...
                .map(|choice| {
                    ChatStreamChoice {
                        ...
                        delta: ChatDelta {
                            ...
                            function_call: choice["delta"]["function_call"].as_object().and_then(|_| {
                                serde_json::from_value(choice["delta"]["function_call"].clone()).ok()
                            }),
