...
impl AzureEmbeddingUtils {
    ...
    pub fn transform_response(response: Value, model: &str) -> Result<EmbeddingResponse, AzureError> {
        let data = response["data"]
            ...
            .map(|item| {
                let embedding = item["embedding"]
                    .as_array()
                    .ok_or_else(|| ProviderError::serialization("azure", "Missing embedding array"))?
                    .iter()
