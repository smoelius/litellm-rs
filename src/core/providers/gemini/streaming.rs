                    ...
                    None => {
                        ...
                        if !buffer.trim().is_empty() {
                            if let Some(event) = GeminiSSEParser::parse_event(&buffer) {
                                let new_state = (lines, buffer, chunk_id, model);
                                match GeminiSSEParser::transform_to_chat_chunk(&event, &new_state.3, &new_state.2) {
...
mod tests {
    ...
    async fn test_stream_creation() {
        ...
        assert_eq!(chunk3.choices[0].finish_reason.as_ref().unwrap(), &crate::core::types::responses::FinishReason::Stop);
    }
}